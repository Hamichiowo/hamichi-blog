...
發現第 152 行最後放了一個奇怪的東西，為什麼會引入照片 ???
```php
<?php include "img/flare-on.png" ?>
```

直接看照片裡是不是偷寫了甚麼，以下是照片最後藏的 code
```php
...
被混淆過，所以我們把`eval($do_me);`改成`echo ($do_me);`並跑起來
```php
...
def decode_escape(m):
    ...
    if x.startswith(r"\x"):
        return chr(int(x[2:], 16))   # hex

    return chr(int(x[1:], 10))       # decimal

result = re.sub(r"\\x[0-9a-fA-F]{2}|\\[0-9]{2,3}", decode_escape, s)
